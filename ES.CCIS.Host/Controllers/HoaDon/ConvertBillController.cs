...
namespace ES.CCIS.Host.Controllers.HoaDon
{
    ...
    public class ConvertBillController : ApiBaseController
    {
        ...
        public HttpResponseMessage Search_Bill(string customerCode, DateTime? date, [DefaultValue(1)] int pageNumber)
        {
            try
            {
                using (var db = new CCISContext())
                {
                    ...
                    var query = (from b_eTro in db.Bill_ElectronicBill
                                 join b_tInv in db.Bill_TaxInvoice on b_eTro.BillId equals b_tInv.BillId into table1
                                 from tbl1 in table1.DefaultIfEmpty()
                                 join b_eLec in db.Bill_ElectricityBill on b_eTro.BillId equals b_eLec.BillId into table2
                                 from tbl2 in table2.DefaultIfEmpty()
                                 join b_adjust in db.Bill_ElectricityBillAdjustment on b_eTro.BillId equals b_adjust.BillAdjustmentId into table3
                                 ...
                                 select new BillConvertViewModel
                                 {
                                     ...
                                     Year = b_eTro.Year,
                                     isTransformBill =
                                     tbl3 != null ? db.Bill_ElectronicBill.Any(b => b.BillId == tbl3.BillId && b.ConvertUser == null && b.ConvertDate == null)
        ...
        public HttpResponseMessage BillConvert(EditBillContentModel model)
        {
            try
            {
                using (var db = new CCISContext())
                {
                    ...
                    string billType = db.Bill_ElectronicBill.Where(r => r.BillId == model.BillId).Select(r => r.BillType).FirstOrDefault();

                    if (!string.IsNullOrEmpty(billType))
                    {
                        var bill = db.Bill_TaxInvoice.Where(r => r.TaxInvoiceId == model.BillId).FirstOrDefault();
        ...
        //        {
        //            // Lấy file XML
        //            string getFileXML = db.Bill_ElectronicBill
        //                .Where(item => item.BillId == BIllId)
        //                .Select(item => item.SignValue_XML)
        //                .FirstOrDefault();
        //            // Lấy file XSL
        //            string getFileXSL = db.Category_ElectronicBillForm
        ...
        public HttpResponseMessage UpdatePrinter(int billId)
        {
            try
            {
                using (var db = new CCISContext())
                {
                    ...
                    var billElectronic = db.Bill_ElectronicBill.FirstOrDefault(r => r.BillId == billId);
            ...
            try
            {
                using (var db = new CCISContext())
                {
                    using (var dbContextTransaction = db.Database.BeginTransaction())
                    {
                        try
                        {
                            var bill = db.Bill_ElectronicBill.FirstOrDefault(r => r.BillId == BillId);
