    ...
    public class LiabilitiesController : ApiBaseController
    {
        ...
        public HttpResponseMessage InvoiceValidation(DateTime? saveDate, [DefaultValue(1)] int term)
        {
            try
            {
                ...
                var SaveIndex =
                    _dbContext.Index_CalendarOfSaveIndex.Where(item =>
                ...
                for (var i = 0; i < SaveIndex.Count; i++)
                {
                    ...
                    List<Index_CalendarOfSaveIndexModel> list =
                        _dbContext.Index_CalendarOfSaveIndex.Where(item => item.FigureBookId.Equals(FigureBookId) && item.Term.Equals(term) && item.Month.Equals(saveDate.Value.Month) && item.Year.Equals(saveDate.Value.Year) && lstDepartmentIds.Contains(item.DepartmentId))
                ...
                if (CalendarOfSave.Count > 0)
                {
                    for (int i = 0; i < CalendarOfSave.Count; i++)
                    {
                        ...
                        var listBills
                            = _dbContext.Bill_ElectricityBill.Where(item =>
                                       FigureBookId == item.FigureBookId &&
        ...
        public HttpResponseMessage SaveInvoiceValidation(int editing, int Term, int Month, int Year, int DepartmentId)
        {
            ...
            using (var _dbContextTransaction = _dbContext.Database.BeginTransaction())
            {
                try
                {
                    ...
                    var checkTotal = _dbContext.Bill_ElectricityBillDetail.Where(
                        item =>
                    ...
                    Index_CalendarOfSaveIndex.UpdataStatus(SaveIndex, _dbContext);
        ...
        public HttpResponseMessage LiabilitiesManager_ByCustomerId(LiabilitiesManager_ByCustomerIdInput input)
        {
            try
            {
                ...
                var listLiabilities =
                    _dbContext.Liabilities_TrackDebt.Where(item => item.BillId.Equals(input.BillId))
                ...
                for (var m = 0; m < allList.Count; m++)
                {
                    ...
                    decimal excessMoney = ValueExcessMoney(customerId, departmentId, _dbContext);
                    allList[m].ExcessMoney = excessMoney;
                }
                name_add = _dbContext.Concus_Customer.Where(item => item.CustomerId.Equals(input.CustomerId)).FirstOrDefault();
                ...
                var JobLatch = _dbContext.Liabilities_JobLatch
        ...
        public HttpResponseMessage SaveLiabilities_TrackDebt_TaxInvoice(SaveLiabilities_TrackDebt_TaxInvoiceInput input)
        {
            using (var _dbContextTransaction = _dbContext.Database.BeginTransaction())
            {
                try
                {
                    ...
                    _dbContext.Liabilities_TrackDebt_TaxInvoice.Where(
                    ...
                    for (var i = 0; i < listLiabilitiesTrackDebt.Count; i++)
                    {
                        ...
                        Insert_LiabilitiesTrackDebt_TaxInvoiceLog(listLiabilitiesTrackDebt[i].Liabilities_TaxInvoiceId, _dbContext, createUser, input.PaymentDate, paymentMoney);
                        trackDebt.Updata_Liabilities_TrackDebt_TaxInvoice(listLiabilitiesTrackDebt[i], _dbContext);
                    }
                    #endregion
                    ...
                    int checkLiabilitiesExcessMoney =
                        _dbContext.Liabilities_ExcessMoney_TaxInvoice.Where(item => item.CustomerId.Equals(input.CustomerId)).Count();
                    if (checkLiabilitiesExcessMoney > 0)
                    {
                        // thực hiện lưu vào log trước Liabilities_ExcessMoney_Log
                        Insert_LiabilitiesExcessMoney_TaxInvoiceLog(input.CustomerId, _dbContext, createUser);
                        ...
                        trackDebt.Edit_Liabilities_ExcessMoney_TaxInvoice(ExcessMoney, _dbContext);
                    }
                    else
                    {
                        var customerCode =
                            (_dbContext.Concus_Customer.Where(item => item.CustomerId.Equals(input.CustomerId))
                        ...
                        trackDebt.Insert_Liabilities_ExcessMoney_TaxInvoice(ExcessMoney, _dbContext);
                    }
                    #endregion
                    ...
                    var nameKh = (_dbContext.Concus_Customer.Where(item => item.CustomerId.Equals(input.CustomerId))
                            .Select(item => item.CustomerCode).FirstOrDefault());
                    if (checkshown != null)
                    {
                        parames = nameKh;
                    ...
                    // Tự động gửi email hóa đơn đến KH
                    Business_Administrator_Parameter vParameters = new Business_Administrator_Parameter();
                    var taxinvoice = _dbContext.Bill_TaxInvoice.Where(x => x.TaxInvoiceId == input.TaxInvoiceId).Select(x => new { x.BillId, x.DepartmentId }).FirstOrDefault();
        ...
        public HttpResponseMessage LiabilitiesManager_TaxInvoice(LiabilitiesManager_TaxInvoiceInput input)
        {
            try
            {
                ...
                var listLiabilities =
                    _dbContext.Liabilities_TrackDebt_TaxInvoice.Where(item => item.CustomerId.Equals(input.CustomerId) && item.TaxInvoiceId.Equals(input.TaxInvoiceId) && (item.Status != 2 && item.Status != 3))
                        .Select(item => new Liabilities_TrackDebt_TaxInvoiceModel
                        {
                            Liabilities_TaxInvoiceId = item.Liabilities_TaxInvoiceId,
                            TaxInvoiceId = item.TaxInvoiceId,
                ...
                var listTrackDebt =
                    _dbContext.Liabilities_TrackDebt_TaxInvoice.Where(item => item.CustomerId.Equals(input.CustomerId) && (item.Debt != 0 || item.TaxDebt != 0) && (item.Status != 2 && item.Status != 3))
                        .Select(item => new Liabilities_TrackDebt_TaxInvoiceModel
                        {
                            Liabilities_TaxInvoiceId = item.Liabilities_TaxInvoiceId,
                            TaxInvoiceId = item.TaxInvoiceId,
                ...
                for (var m = 0; m < allList.Count; m++)
                {
                    int departmentId = Convert.ToInt32(allList[m].DepartmentId);
                    int customerId = Convert.ToInt32(allList[m].CustomerId);
                    decimal excessMoney = ValueExcessMoneyTaxInvoice(customerId, departmentId, _dbContext);
                    allList[m].ExcessMoney = excessMoney;
        ...
        public HttpResponseMessage DebtCancellation([DefaultValue("")] string Name, DateTime? saveDate, [DefaultValue(0)] int Term, [DefaultValue(0)] int FigureBookId)
        {
            try
            {
                ...
                if (saveDate != null)
                {
                    ...
                    var listTrackDebt = (from no in _dbContext.Liabilities_TrackDebt
                                         join lg in _dbContext.Liabilities_TrackDebt_Log
                                         on no.BillId equals lg.BillId
                                         join tg in _dbContext.Liabilities_JobLatch
                                         ...
                                         select new Liabilities_TrackDebtModel
                                         {
                                             ...
                                             BillingStatus = 0,
                                             AdjustmentType = _dbContext.Bill_ElectricityBillAdjustment.Where(item => item.BillId.Equals(no.BillId)).Select(item => item.AdjustmentType).FirstOrDefault(),
        ...
        public HttpResponseMessage GetLiabilities_TrackDebt_Log(int BillId, int CustomerId)
        {
            try
            {
                ...
                var listLiabilities =
                    _dbContext.Liabilities_TrackDebt_Log.Where(item => item.BillId.Equals(BillId))
                        .Select(item => new Liabilities_TrackDebt_LogModel
                        {
                            ...
                            FundsGenerated = item.FundsGenerated,
                            TaxesIncurred = item.TaxesIncurred,
                            TongTienNo = (_dbContext.Liabilities_TrackDebt.Where(a => a.BillId.Equals(BillId)).Select(a => a.FundsGenerated).FirstOrDefault()),
                            TongThueNo = (_dbContext.Liabilities_TrackDebt.Where(a => a.BillId.Equals(BillId)).Select(a => a.TaxesIncurred).FirstOrDefault()),
                            PointCode = (_dbContext.Concus_ServicePoint.Where(a => a.PointId.Equals(item.PointId)).Select(a => a.PointCode).FirstOrDefault()),
                            Day = item.DateDot.Day,
                            Month = item.DateDot.Month,
                            Year = item.DateDot.Year,
                            Payment = item.Payment,
                            Name = (_dbContext.UserProfile.Where(a => a.UserId.Equals(item.CreateUser)).Select(a => a.UserName)).FirstOrDefault()
                        }).OrderByDescending(item => item.Id_LiabilitiesIdLog).ThenBy(item => item.DateDot).ToList();
                // sau khi kết thúc vòng lặp, nếu list danh sách nợ còn thì phải cho hết vào danh sách tổng
                allList.AddRange(listLiabilities);
                // thực hiện vòng  lặp để lấy ra số tiền thừa của khách hàng
                name_add = _dbContext.Concus_Customer.Where(item => item.CustomerId.Equals(CustomerId)).FirstOrDefault();
                var Liabilities_JobLatch = _dbContext.Liabilities_JobLatch
                ...
                var figureBookId = _dbContext.Bill_ElectricityBill.Where(item => item.BillId == BillId).FirstOrDefault().FigureBookId;
        ...
        public HttpResponseMessage DebtCancellation_TrackDebt_Log(int Id_LiabilitiesIdLog)
        {
            try
            {
                ...
                var listLiabilities = _dbContext.Liabilities_TrackDebt_Log
                ...
                if (Liabilities_JobLatch != 0)  //trong tháng hoạch toán -> cho xóa
                {
                    ...
                    var liabilitiesTrackDebt = _dbContext.Liabilities_TrackDebt.FirstOrDefault(item => item.DepartmentId == listLiabilities.DepartmentId
                    ...
                    if (listLiabilities != null && liabilitiesTrackDebt != null)
                    {
                        using (var _dbContextTransaction = _dbContext.Database.BeginTransaction())
                        {
                            try
                            {
                                ...
                                var lstExcessMoneyLog = _dbContext.Liabilities_ExcessMoney_Log.FirstOrDefault(item => item.BillId.Equals(listLiabilities.BillId)
                                ...
                                if (lstExcessMoneyLog != null)  //nếu có liên quan tiền thừa?
                                {
                                    ...
                                    var vExcessMoney = _dbContext.Liabilities_ExcessMoney
                                    ...
                                    //xóa log tiền thừa
                                    _dbContext.Liabilities_ExcessMoney_Log.Remove(lstExcessMoneyLog);
                                    _dbContext.SaveChanges();
                                }

                                //xóa dòng log đã thêm gần đây nhất đi
                                _dbContext.Liabilities_TrackDebt_Log.Remove(listLiabilities);
                                _dbContext.SaveChanges();
                                trangthai = true;
                                _dbContextTransaction.Commit();
                            }
        ...
        public HttpResponseMessage TaxInvoiceCancellation([DefaultValue("")] string Name, DateTime? saveDate, [DefaultValue(0)] int FigureBookId)
        {
            try
            {
                ...
                AllTaxInvoiceCancellation(TongKH, TongTien, allList, _dbContext, Name.Trim(), saveDate, FigureBookId);
        ...
        public HttpResponseMessage GetLiabilities_TrackDebt_TaxInvoice_Log(int TaxInvoiceId, int CustomerId)
        {
            try
            {
                ...
                int departmentId = TokenHelper.GetDepartmentIdFromToken();
                var listLiabilities =
                    _dbContext.Liabilities_TrackDebt_TaxInvoice_Log.Where(item => item.TaxInvoiceId.Equals(TaxInvoiceId))
                        .Select(item => new Liabilities_TrackDebt_TaxInvoice_LogModel
                        {
                            ...
                            FundsGenerated = item.FundsGenerated,
                            TaxesIncurred = item.TaxesIncurred,
                            TongTienNo = (_dbContext.Liabilities_TrackDebt_TaxInvoice.Where(a => a.TaxInvoiceId.Equals(TaxInvoiceId)).Select(a => a.FundsGenerated).FirstOrDefault()),
                            TongThueNo = (_dbContext.Liabilities_TrackDebt_TaxInvoice.Where(a => a.TaxInvoiceId.Equals(TaxInvoiceId)).Select(a => a.TaxesIncurred).FirstOrDefault()),
                            Day = item.DateDot.Day,
                            Month = item.DateDot.Month,
                            Year = item.DateDot.Year,
                            Payment = item.Payment,
        ...
        public HttpResponseMessage DebtCancellation_TaxInvoice_Log(int Id_LiabilitiesTaxInvoiceLog)
        {
            try
            {
                ...
                var listLiabilities =
                    _dbContext.Liabilities_TrackDebt_TaxInvoice_Log.Where(item => item.Id_LiabilitiesTaxInvoiceLog.Equals(Id_LiabilitiesTaxInvoiceLog))
                        .FirstOrDefault();
                var Liabilities_JobLatch = _dbContext.Liabilities_JobLatch
                    .Where(item => item.DepartmentId.Equals(departmentId) &&
                                   item.Month.Equals(listLiabilities.Month) &&
                ...
                {
                    // thực hiện vòng  lặp để lấy ra số tiền thừa của khách hàng
                    var liabilitiesTrackDebt = _dbContext.Liabilities_TrackDebt_TaxInvoice
                    ...
                    var liabilitiesExcessMoney = _dbContext.Liabilities_ExcessMoney_TaxInvoice
                    ...
                    var ExcessMoneyLog_Id = _dbContext.Liabilities_ExcessMoney_TaxInvoice_Log.OrderByDescending(item => item.ExcessMoneyLogTaxInvoice_Id)
                    ...
                    if (listLiabilities != null && liabilitiesTrackDebt != null)
                    {
                        ...
                        using (var _dbContextTransaction = _dbContext.Database.BeginTransaction())
                        {
                            try
                            {
                                ...
                                {
                                    TaxInvoice.Rollback_ExcessMoneyTaxInvoice(money, _dbContext);
                                }
                                //bước 2 : trả lại 2 cột tiền nợ , thuế nợ trước đây
                                TaxInvoice.Rollback_TaxInvoice(TrackDebt, _dbContext);
                                // bước 3 : xóa dòng log đã thêm gần đây nhất đi
                                TaxInvoice.DeleteLiabilities_TrackDebt_TaxInvoice_Log(TrackDebt_Log, _dbContext);
                                // Bước 4 : xóa dòng tiền thừa trong bảng log đi
                                ExcessMoney_TaxInvoiceLog.DeleteLiabilities_ExcessMoney_TaxInvoice_Log(ExcessMoney_Log, _dbContext);
                                trangthai = true;
                                _dbContextTransaction.Commit();
                            }
                            catch (Exception ex)
        ...
        public HttpResponseMessage SaveMonthlyDebtConfirmation(int departmentId)
        {
            try
            {
                ...
                var liabilitiesJobLatch = _dbContext.Liabilities_JobLatch
                    .Where(item => item.DepartmentId.Equals(departmentId)).ToList().FirstOrDefault();
                ...
                var lstRootBook = _dbContext.Category_FigureBook.Where(x => x.IsRootBook == true && x.DepartmentId == departmentId).Select(x => x.FigureBookId).ToList();

                //1-check có sổ nào chưa ký hóa đơn hay không nếu chưa ký không cho chuyển tháng
                var listFigureBook = _dbContext.Index_CalendarOfSaveIndex.Where(item => item.Status < 11 && item.Status > 1 && item.Month == month && item.Year == year
                                                                    && item.DepartmentId == departmentId && !lstRootBook.Contains(item.FigureBookId)).FirstOrDefault();

                if (listFigureBook != null)
                {
                    var soKhongCoHoaDon = _dbContext.Liabilities_TrackDebt.Any(item => listFigureBook.FigureBookId == item.FigureBookId && item.Month == month && item.Year == year);
                    if (soKhongCoHoaDon)
                    {
                        trangthai = false;
                        var strSo = _dbContext.Category_FigureBook.Where(o => o.DepartmentId == departmentId && o.FigureBookId == listFigureBook.FigureBookId).FirstOrDefault();
                ...
                var listBillAdjustment = _dbContext.Bill_ElectricityBillAdjustment
                ...
                _dbContext.Database.SqlQuery<List<int>>("[_dbContexto].[SaveMonthlyDebtConfirmation] @departmentId, @month, @year", new SqlParameter("departmentId", departmentId), new SqlParameter("month", month), new SqlParameter("year", year))
                           .FirstOrDefault();
                ...
                model.DepartmentId = departmentId;
                saveJobLatch.UpdateLiabilities_JobLatch(_dbContext, model);
        ...
        public HttpResponseMessage CheckTime_MonthOfDebt(string testingDate)
        {
            try
            {
                ...
                var departmentId = TokenHelper.GetDepartmentIdFromToken();
                var thangcongno = _dbContext.Liabilities_JobLatch.Where(item => item.DepartmentId.Equals(departmentId)).FirstOrDefault();
                if (thangcongno != null)
                {
        ...
        public HttpResponseMessage Sms([DefaultValue(0)] int departmentId, [DefaultValue(0)] int Term, DateTime? saveDate, [DefaultValue(0)] int FigureBookId, [DefaultValue(0)] int smsvalue, string thongbao)
        {
            try
            {
                ...
                {
                    // thong bao tien dien
                    var dskh = _dbContext.Bill_ElectricityBill
                    .Where(item => item.FigureBookId == FigureBookId && item.Term == Term &&
                                   ...
                                   && item.Year == saveDate.Value.Year).Select(item => new Sms
                                   {
                                       ...
                                       Month = item.Month,
                                       Year = item.Year,
                                       OldValue = (_dbContext.Index_Value.Where(a => a.CustomerId == item.CustomerId
                                            && a.Month == item.Month && a.Year == item.Year
                                            && a.IndexType == EnumMethod.LoaiChiSo.DDK)
                                            .Select(a => a.OldValue)).FirstOrDefault(),
                                       NewValue = (_dbContext.Index_Value.Where(a => a.CustomerId == item.CustomerId
                                            && a.Month == item.Month && a.Year == item.Year
                                            && a.IndexType == EnumMethod.LoaiChiSo.DDK)
                ...
                {
                    // thong bao tien dien
                    var dskh = _dbContext.Liabilities_TrackDebt
                    .Where(item => item.FigureBookId == FigureBookId && item.Term == Term &&
                                   ...
                                   .Select(item => new Sms
                                   {
                                       ...
                                       CustomerCode = item.CustomerCode,
                                       BillId = item.BillId,
                                       PhoneCustomerCare = (_dbContext.Concus_Customer.Where(a => a.CustomerId == item.CustomerId).Select(a => a.PhoneCustomerCare)).FirstOrDefault(),
                                       Term = item.Term,
                                       Month = item.Month,
                                       Year = item.Year,
                                       OldValue = (_dbContext.Index_Value.Where(a => a.CustomerId == item.CustomerId
                                       ...
                                       NewValue = (_dbContext.Index_Value.Where(a => a.CustomerId == item.CustomerId
                                            ...
                                            && a.IndexType == EnumMethod.LoaiChiSo.DDK)
                                            .Select(a => a.NewValue)).FirstOrDefault(),
                                       RealElectricityIndex = (_dbContext.Bill_ElectricityBill.Where(a => a.CustomerId == item.CustomerId && a.BillId == item.BillId).Select(a => a.ElectricityIndex)).FirstOrDefault(),
                ...
                else
                {
                    ...
                    var listContractid = _dbContext.Concus_ServicePoint.Where(item => item.FigureBookId == FigureBookId && item.Status == true && item.IsRootPoint == false).Select(i2 => i2.ContractId).ToList();
                    if (listContractid != null)
                    {
                        var listCustomerId = _dbContext.Concus_Contract.Where(item => listContractid.Contains(item.ContractId)).Select(i2 => i2.CustomerId).ToList();
                        list = _dbContext.Concus_Customer
        ...
        //                            var dskh = _dbContext.Bill_ElectricityBillDetail
        ...
        //                            var indexInfor = _dbContext.Index_Value.Where(a => a.CustomerId == dskh.CustomerId
        //                                                            && a.Month == dskh.Month && a.Year == dskh.Year
        //                                                            && a.IndexType != "DUP")
        //                                                            .OrderBy(a2 => a2.IndexId)
        //                                                            .ToList();
        //                            var billInfor = _dbContext.Bill_ElectricityBill.Where(item => item.BillId == Billid).FirstOrDefault();
        ...
        //                            _dbContext.Sms_Track_Customer.Add(sk);
        //                            _dbContext.SaveChanges();
        //                        }
        //                        catch (Exception ex)
        //                        {
        ...
        public HttpResponseMessage LiabilitiesManagerBy(LiabilitiesManagerByInput input)
        {
            using (var _dbContextTransaction = _dbContext.Database.BeginTransaction())
            {
                try
                {
                    ...
                    var listLiabilitiesTrackDebt =
                        _dbContext.Liabilities_TrackDebt.Where(
                    ...
                    var lstCalendarOfSaveIndex = _dbContext.Index_CalendarOfSaveIndex.Where(
                    ...
                    foreach (var bill in listLiabilitiesTrackDebt)
                    {
                        ...
                        var BillElectricityBillEndDate = _dbContext.Bill_ElectricityBill.FirstOrDefault(item => item.BillId == bill.BillId)?.EndDate;
                        var BillAdjustElectricityBillEndDate = _dbContext.Bill_ElectricityBillAdjustment.FirstOrDefault(item => item.BillId == bill.BillId)?.EndDate;
                        ...
                        if (endDate <= paymentDate)
                        {
                            ...
                            bill.TaxDebt = 0;
                            bill.Status = 1;
                            TrackDebt.Updata_Liabilities_TrackDebt(bill, _dbContext, createUser, paymentDate);
                            var trackdept = _dbContext.Liabilities_TrackDebt.FirstOrDefault(o => o.BillId == bill.BillId);
        ...
        {
            using (var _dbContextT = _dbContext.Database.BeginTransaction())
            {
                try
                {
                    var IndexCalendar = _dbContext.Index_CalendarOfSaveIndex
                    ...
                    {
                        var lstBill = _dbContext.Bill_ElectricityBill
                                                     ...
                                                     .Select(item => new Bill_ElectricityBillModel
                                                     {
                                                         ...
                                                         VAT = item.VAT,
                                                         BillType = item.BillType,
                                                         PointId = _dbContext.Bill_ElectricityBillDetail.Where(x => x.Term == input.Term && x.Month == input.Month
                                                                     && x.Year == input.Year && x.FigureBookId == input.FigurebookId)
                                                                     .Select(x => x.PointId).Distinct().FirstOrDefault()
                                                     })
                                                     .ToList();

                        Compare_Liabilities_TrackDebt(lstBill, _dbContext);
                    }
                        ...
                        if (input.MoveDept == true && IndexCalendar.Status == (int)(DefaultBusinessValue.StatusCalendarOfSaveIndex.ConfirmData))
                        {
                            var lstBill = _dbContext.Bill_ElectricityBill
                                ...
                                .Select(item => new Bill_ElectricityBillModel
                                {
                                    ...
                                    VAT = item.VAT,
                                    BillType = item.BillType,
                                    PointId = _dbContext.Bill_ElectricityBillDetail.Where(x => x.Term == ite.Term && x.Month == input.ConfirmDate.Month
        ...
        //Chuyển sang công nợ
        public void Compare_Liabilities_TrackDebt(List<Bill_ElectricityBillModel> billElectricityBill, CCISContext _dbContext)
        {
            ...
            for (int j = 0; j < billElectricityBill.Count; j++)
            {
                ...
                {
                    var Name =
                        _dbContext.Concus_Customer.Where(item => item.CustomerId.Equals(lisBill.CustomerId))
                    ...
                    modelTrackDebt.FigureBookId =
                        _dbContext.Bill_ElectricityBillDetail.Where(item => item.BillId.Equals(lisBill.BillId))
                            .Select(item => item.FigureBookId)
                            .FirstOrDefault();
                    // chưa có thì insert vao
                    TrackDebt.Insert_Liabilities_TrackDebt(modelTrackDebt, _dbContext);
                }
            }
        }
        #endregion
        ...
        public HttpResponseMessage Confirm_TrackDebt_Adjustment(DateTime? confirmDate)
        {
            try
            {
                using (var _dbContext = new CCISContext())
                {
                    var lstBill = _dbContext.Bill_ElectricityBillAdjustment
                                    ...
                                    .Select(item => new Bill_ElectricityBillAdjustmentModel
                                    {
                                        ...
                                        DepartmentName = _dbContext.Administrator_Department.Where(x => x.DepartmentId == item.DepartmentId)
        ...
        public HttpResponseMessage Move_TrackDebt_Adjustment(decimal[] lstBill, DateTime confirmDate)
        {
            using (var _dbContext = new CCISContext())
            {
                using (var _dbContextTran = _dbContext.Database.BeginTransaction())
                {
                    try
                    {
                        foreach (var ite in lstBill)
                        {
                            var lstBillAdjustment = _dbContext.Bill_ElectricityBillAdjustment
                            ...
                            Compare_Liabilities_TrackDebt_Bill_Adjustment(lstBillAdjustment, _dbContext);

                            var bill = _dbContext.Bill_ElectricityBillAdjustment.Where(x => x.BillId == ite).FirstOrDefault();
                            bill.Status = 1;
                            _dbContext.SaveChanges();
                        }

                        _dbContextTran.Commit();
        ...
        public void Compare_Liabilities_TrackDebt_Bill_Adjustment(List<Bill_ElectricityBillAdjustmentModel> lstBill, CCISContext _dbContext)
        {
            int createUser = TokenHelper.GetUserIdFromToken();
            // thực hiện vòng lặp hóa đơn để kiểm tra xem có bên bảng theo dõi nợ không, nếu có thì thông báo, nếu không thì đông bộ dữ liệu sang
            foreach (var item in lstBill)
            {
                ...
                if (Count <= 0)
                {
                    ...
                    modelTrackDebt.PointId = _dbContext.Bill_ElectricityBillAdjustmentDetail
                                            .Where(x => x.BillId == item.BillId)
                                            .Select(x => x.PointId)
                                            .FirstOrDefault();
                    modelTrackDebt.FigureBookId =
                        _dbContext.Bill_ElectricityBillAdjustmentDetail.Where(x => x.BillId == item.BillId)
        ...
        #region Commons
        protected decimal ValueExcessMoney(int CustomerId, int DepartmentId, CCISContext _dbContext)
        {
            var value =
                _dbContext.Liabilities_ExcessMoney.Where(
        ...
        {
            var excessMoney =
                _dbContext.Liabilities_ExcessMoney.Where(item => item.CustomerId.Equals(customerId))
                    .Select(item => item.ExcessMoney)
                    .FirstOrDefault();
            return excessMoney;
        }

        protected void Update_LiabilitiesExcessMoney(Liabilities_TrackDebtModel TrackDept, int iUser, DateTime PayDate, CCISContext _dbContext, decimal excessMoneyUsed, int logTrack)
        {
            if (excessMoneyUsed == 0)
                return;
            var ExcessMoney = _dbContext.Liabilities_ExcessMoney.Where(item => item.CustomerId.Equals(TrackDept.CustomerId)).FirstOrDefault();
            ...
            else
            {
                ...
                _dbContext.Liabilities_ExcessMoney.Add(ExcessMoney);
                _dbContext.SaveChanges();
            }
            ...
            _dbContext.Liabilities_ExcessMoney_Log.Add(target);
            _dbContext.SaveChanges();
        }
        ...
        private Dictionary<string, object> PaymentTrackDebt(int CustomerId, string namemoney, int Bill, DateTime paymentDate, string PaymentMethodsCode)
        {
            ...
            using (var _dbContextTransaction = _dbContext.Database.BeginTransaction())
            {
                ...
                try
                {
                    ...
                    //lấy ra ngày cuối kỳ 
                    var endDate = _dbContext.Index_CalendarOfSaveIndex.Where(
                    ...
                    if (paymentDate >= endDate)
                    {
                        ...
                        if (money > 0) //trường hợp phổ biến, hóa đơn tiền > 0
                        {
                            if (paymentMoney >= debt + taxDebt)  //nếu tiền nộp lần này nhiều hơn hoặc bằng tiền hóa đơn chấm thì chỉ sử dụng tiền nộp thôi
                            {
                                ...
                                //Cập nhật bảng nợ (xóa nợ)
                                logTrack = TrackDebt.Updata_Liabilities_TrackDebt(liabilitiesTrackDebt, _dbContext, createUser, paymentDate);
                                if (paymentMoney > debt + taxDebt)  //nếu tiền nộp lớn hơn tiền nợ thì cập nhật thêm vào tiền thừa
                                {
                                    Update_LiabilitiesExcessMoney(liabilitiesTrackDebt, createUser, paymentDate, _dbContext, paymentMoney - debt - taxDebt, logTrack);
                                }
                            }
                            else  //trường hợp tiền nộp ít hơn tiền phải nộp thì sử dụng tiền nộp và tiền thừa nếu có
                            {
                                ...
                                if (excessMoney > 0) //tiền thừa sử dụng sau (nếu có)
                                {
                                    ...
                                    //Cập nhật bảng nợ (xóa nợ)
                                    logTrack = TrackDebt.Updata_Liabilities_TrackDebt(liabilitiesTrackDebt, _dbContext, createUser, paymentDate);
                                    Update_LiabilitiesExcessMoney(liabilitiesTrackDebt, createUser, paymentDate, _dbContext, (excessMoneyUsed * -1), logTrack);
                                }
                            }
                        }
                        ...
                        #region Kiểm tra xem khách hàng còn nợ tiền hay không, nếu có thì hiển thị khách hàng lên
                        // check xem nguoi dung nay con cong no khong, neu con thi hien thi nguoi dung nay len, khong thi thoi
                        var checkshown = _dbContext.Liabilities_TrackDebt.Where(
        ...
        protected void AllLiabilities_TrackDebt_TaxInvoice(int TongKH, decimal TongTien, List<Liabilities_TrackDebt_TaxInvoiceModel> allList, int departmentId)
        {
            using (var _dbContext = new CCISContext())
            {
                ...
                var listLiabilitiesTaxInvoice =
                    _dbContext.Liabilities_TrackDebt_TaxInvoice.Where(item => (item.Debt != 0 || item.TaxDebt != 0) && item.Status == 0 && item.DepartmentId == departmentId)
                        .Select(item => new Liabilities_TrackDebt_TaxInvoiceModel
                        {
                            Liabilities_TaxInvoiceId = item.Liabilities_TaxInvoiceId,
                            TaxInvoiceId = item.TaxInvoiceId,
                ...
                var listTrackDebtTaxInvoice =
                    _dbContext.Liabilities_TrackDebt_TaxInvoice.Where(item => item.Status == 1 && (item.Debt != 0 || item.TaxDebt != 0) && item.DepartmentId == departmentId)
                        .Select(item => new Liabilities_TrackDebt_TaxInvoiceModel
                        {
                            Liabilities_TaxInvoiceId = item.Liabilities_TaxInvoiceId,
                            TaxInvoiceId = item.TaxInvoiceId,
                ...
                for (var m = 0; m < allList.Count; m++)
                {
                    ...
                    int customerId = Convert.ToInt32(allList[m].CustomerId);
                    decimal excessMoney = ValueExcessMoneyTaxInvoice(customerId, _departmentId, _dbContext);
                    allList[m].ExcessMoney = excessMoney;
                    if ((allList[m].Debt != 0 || allList[m].TaxDebt != 0) && allList[m].Status == 1)
                    {
                        allList[m].TaxInvoiceStatus = 0;
        ...
        protected decimal ValueExcessMoneyTaxInvoice(int CustomerId, int DepartmentId, CCISContext _dbContext)
        {
            var value =
                _dbContext.Liabilities_ExcessMoney_TaxInvoice.Where(
                    item => item.CustomerId.Equals(CustomerId) && item.DepartmentId.Equals(DepartmentId))
                    .Select(item => item.ExcessMoney)
                    .FirstOrDefault();
            return value;
        ...
        protected void CustomerLiabilities_TrackDebt_TaxInvoice(int TongKH, decimal TongTien, List<Liabilities_TrackDebt_TaxInvoiceModel> allList, string Name, string CustomerCode)
        {
            using (var _dbContext = new CCISContext())
            {
                ...
                // lấy ra danh sách row có trạng thái = 0
                var listLiabilities =
                    _dbContext.Liabilities_TrackDebt_TaxInvoice.Where(item => ((item.Name.Contains(Name.Trim()) || item.CustomerCode.Contains(CustomerCode.Trim())) && item.Debt > 0) && (item.Status != 2 && item.Status != 3))
                        .Select(item => new Liabilities_TrackDebt_TaxInvoiceModel
                        {
                            Liabilities_TaxInvoiceId = item.Liabilities_TaxInvoiceId,
                            TaxInvoiceId = item.TaxInvoiceId,
                ...
                // lay ra danh sách khách hàng còn nợ (check : Debt != 0  hoặc TaxDebt != 0)
                var listTrackDebt =
                    _dbContext.Liabilities_TrackDebt_TaxInvoice.Where(item => (item.Status != 0 && item.Status != 2 && item.Status != 3) && (item.Debt != 0 || item.TaxDebt != 0) && (item.Name.Contains(Name.Trim()) || item.CustomerCode.Contains(CustomerCode.Trim())))
                        .Select(item => new Liabilities_TrackDebt_TaxInvoiceModel
                        {
                            Liabilities_TaxInvoiceId = item.Liabilities_TaxInvoiceId,
                            TaxInvoiceId = item.TaxInvoiceId,
        ...
        protected decimal GetLiabilities_ExcessMoney_TaxInvoice(int customerId)
        {
            ...
            {
                var excessMoney =
                    _dbContext.Liabilities_ExcessMoney_TaxInvoice.Where(item => item.CustomerId.Equals(customerId))
                        .Select(item => item.ExcessMoney)
                        .FirstOrDefault();
                return excessMoney;
            }
        ...
        protected void Insert_LiabilitiesTrackDebt_TaxInvoiceLog(int Liabilities_TaxInvoiceId, CCISContext _dbContext, int CreateUser, DateTime paymentDate, decimal payment)
        {
            Business_Liabilities_TrackDebt_TaxInvoice_Log trackDebtLog = new Business_Liabilities_TrackDebt_TaxInvoice_Log();
            var TrackDebt =
                _dbContext.Liabilities_TrackDebt_TaxInvoice.Where(item => item.Liabilities_TaxInvoiceId.Equals(Liabilities_TaxInvoiceId)).FirstOrDefault();
            ...
            trackDebtLog.Insert_Liabilities_TrackDebt_TaxInvoice_Log(model, _dbContext);
        }

        protected void Insert_LiabilitiesExcessMoney_TaxInvoiceLog(int CustomerId, CCISContext _dbContext, int CreateUser)
        {
            var ExcessMoney =
                _dbContext.Liabilities_ExcessMoney_TaxInvoice.Where(item => item.CustomerId.Equals(CustomerId)).FirstOrDefault();
            ...
            ExcessMoney_TaxInvoiceLog.Insert_Liabilities_ExcessMoneyTaxInvoice_Log(model, _dbContext);
        }
        #endregion

        #region Hủy bỏ xóa nợ tiền non tải
        protected void AllTaxInvoiceCancellation(int TongKH, decimal TongTien, List<Liabilities_TrackDebt_TaxInvoiceModel> allList, CCISContext _dbContext, string name, DateTime? saveDate, int FigureBookId)
        {
            // khai báo  danh sách chứa cả nợ tồn + pát sinh trong tháng.
            int month = saveDate.Value.Month;
            int year = saveDate.Value.Year;
