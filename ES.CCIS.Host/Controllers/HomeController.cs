    ...
    public class HomeController : ApiBaseController
    {
            ...
            try
            {
                ...
                using (var db = new CCISContext())
                {
                    ...
                    var table_cto = (from table1 in
                                      (from EquipmentMT_ElectricityMeter in db.EquipmentMT_ElectricityMeter.Where(i => lstDepIds.Contains(i.DepartmentId) && i.TestingDate != null && i.EndTestingDate != null && i.EndTestingDate <= date)
                    ...
                    var table_tu = (from table1 in
                                      (from Equipment in db.EquipmentVT_VoltageTransformer.Where(i => lstDepIds.Contains(i.DepartmentId) && i.TestingDate != null && i.EndTestingDate != null && i.EndTestingDate <= date)
                    ...
                    var table_ti = (from table1 in
                                      (from Equipment in db.EquipmentCT_CurrentTransformer.Where(i => lstDepIds.Contains(i.DepartmentId) && i.TestingDate != null && i.EndTestingDate != null && i.EndTestingDate <= date)
                                       where (Equipment.ActionCode == "B"
                                                || Equipment.ActionCode == "A"
                                                || Equipment.ActionCode == "E"
                                             )
                ...
                using (var db = new CCISContext())
                {
                    ...
                    lstContract = db.Concus_Contract.Where(item => item.EndDate < endday && item.EndDate >= DateTime.Now).Select(item => new Concus_ContractModel()
                    {
        ...
        public HttpResponseMessage OverdueMeters()
        {
            try
            {
                ...
                using (var db = new CCISContext())
                {
                    ...
                    //Công tơ quá hạn
                    var countA = db.EquipmentMT_ElectricityMeter.Where(item => item.ActionCode == "B" && lstDepIds.Contains(item.DepartmentId) && item.EndTestingDate < date1).Count();
                    //Công tơ quá hạn trong 3 tháng
                    var countB = db.EquipmentMT_ElectricityMeter.Where(item => item.ActionCode == "B" && lstDepIds.Contains(item.DepartmentId) && item.EndTestingDate >= date1 && item.EndTestingDate < date2).Count();
                    //lấy ra list công tơ trong kho
                    var countE = db.EquipmentMT_ElectricityMeter.Where(item => item.ActionCode == "B" && lstDepIds.Contains(item.DepartmentId) && item.EndTestingDate >= date2).Count();
        ...
        public HttpResponseMessage GetElectricMeterInfo()
        {
            try
            {
                ...
                using (var db = new CCISContext())
                {
                    var list = db.EquipmentMT_ElectricityMeter.Where(item => item.ActionCode != "F" && lstDepIds.Contains(item.DepartmentId)).Select(item => item.ActionCode).ToList();
                    //lấy ra list công tơ trong kho
                    var countA = db.EquipmentMT_ElectricityMeter.Where(item => item.ActionCode == "A" && lstDepIds.Contains(item.DepartmentId)).Select(i => i.ElectricityMeterId).Distinct().Count();
                    //lấy ra list công tơ trong kho
                    var countB = db.EquipmentMT_ElectricityMeter.Where(item => item.ActionCode == "B" && lstDepIds.Contains(item.DepartmentId)).Select(i => i.ElectricityMeterId).Distinct().Count();
                    //lấy ra list công tơ trong kho
                    var countE = db.EquipmentMT_ElectricityMeter.Where(item => item.ActionCode == "E" && lstDepIds.Contains(item.DepartmentId)).Select(i => i.ElectricityMeterId).Distinct().Count();
        ...
        public HttpResponseMessage GetDepartmentInfo(string userName)
        {
            try
            {
                using (var db = new CCISContext())
                {
                    var departmentId = db.UserProfile.Where(item => item.UserName == userName.Trim()).Select(it2 => it2.DepartmentId).FirstOrDefault();
                    string departmentName = db.Administrator_Department.Where(item => item.DepartmentId == departmentId).Select(it2 => it2.DepartmentName).FirstOrDefault();
