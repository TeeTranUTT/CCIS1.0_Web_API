    ...
    public class CurrentTransformerController : ApiBaseController
    {
        ...
        public HttpResponseMessage CurrentTransformer_StockDetail_Testing([DefaultValue(1)] int pageNumber, [DefaultValue("")] string search)
        {
            try
            {
                ...
                {
                    var query = db.EquipmentCT_CurrentTransformer
        ...
        public HttpResponseMessage EquipmentCT_CurrentTransformerManager([DefaultValue(1)] int pageNumber, [DefaultValue("")] string search, [DefaultValue("")] string ActionCode)
        {
            try
            {
                ...
                using (var db = new CCISContext())
                {
                    var query = db.EquipmentCT_CurrentTransformer.Where(item => lstDepartmentIds.Contains(item.DepartmentId))
                        .Select(item => new EquipmentCT_CurrentTransformerModel
                        {
                            CurrentTransformerId = item.CurrentTransformerId,
                            CTCode = item.CTCode,
                            ...
                            CreateDate = item.CreateDate,
                            ActionCode = item.ActionCode,
                            TestingStatus = db.EquipmentCT_Testing.Where(i => i.CurrentTransformerId == item.CurrentTransformerId).Select(i => i.Status).FirstOrDefault()
                        });
        ...
        public HttpResponseMessage LiquidationTI(string CurrentTransformerTI, string ReasonId)
        {
            try
            {
                using (var db = new CCISContext())
                {
                    business_EquipmentCT_CurrentTransformer.UpdateLiquidationTI(CurrentTransformerTI, ReasonId, db);
                ...
                using (var db = new CCISContext())
                {
                    using (var dbContextTransaction = db.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            var typeCode = db.Category_CurrentTransformerType.Where(item => item.CTTypeId == model.CurrentTransformerModel.CTTypeId).FirstOrDefault();
                            ...
                            int currentTransformerId =
                                business_EquipmentCT_CurrentTransformer.AddEquipmentCT_CurrentTransformer(model.CurrentTransformerModel, db);
                            ...
                            equipmentCTTesting.AddEquipmentCT_Testing(model.EquipmentCtTesting, db);
                ...
                using (var db = new CCISContext())
                {
                    using (var dbContextTransaction = db.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            equipmentCTTesting.EditEquipmentCT_Testing(model.EquipmentCtTesting, db);
                            dbContextTransaction.Commit();
        ...
        public HttpResponseMessage EditCurrentTransformer(int editing)
        {
            try
            {
                using (var db = new CCISContext())
                {
                    ...
                    //Lấy thông tin TI
                    var currentTransformer = db.EquipmentCT_CurrentTransformer.Where(item => item.CurrentTransformerId.Equals(editing))
                        .Select(item => new EquipmentCT_CurrentTransformerModel()
                        {
                            CurrentTransformerId = item.CurrentTransformerId,
                            Possesive = item.Possesive,
                    ...
                    var ds = db.EquipmentCT_Testing.Where(item => item.CurrentTransformerId.Equals(editing)).Select(item => new EquipmentCT_CurrentTransformerModel()
                    {
                    ...
                    {
                        var currentTransformerTesting = db.EquipmentCT_Testing.Where(item => item.CurrentTransformerId.Equals(editing)).Select(item => new EquipmentCT_TestingModel()
                        {
                            CurrentTransformerId = item.CurrentTransformerId,
                ...
                using (var db = new CCISContext())
                {
                    using (var dbContextTransaction = db.Database.BeginTransaction())
                    {
                        ...
                        try
                        {
                            ...
                            var typeCode =
                                db.Category_CurrentTransformerType.Where(
                            ...
                            model.CurrentTransformerModel.EndTestingDate = new DateTime(model.CurrentTransformerModel.EndTestingDate.Value.Year, model.CurrentTransformerModel.EndTestingDate.Value.Month, 1);
                            business_EquipmentCT_CurrentTransformer.EditEquipmentCT_CurrentTransformer(model.CurrentTransformerModel, db);
                            //thông tin kiểm định
                            model.EquipmentCtTesting.CurrentTransformerId = model.CurrentTransformerModel.CurrentTransformerId;
                            if (model.EquipmentCtTesting.TestingStatus)
                                model.EquipmentCtTesting.Status = 1;
                            ...
                            equipmentCTTesting.EditEquipmentCT_TestingTi(model.EquipmentCtTesting, db);
        ...
        public HttpResponseMessage DeleteTI(int currentTransformerId)
        {
            try
            {
                using (var db = new CCISContext())
                {
                    var target = db.EquipmentCT_CurrentTransformer.Where(item => item.CurrentTransformerId == currentTransformerId).FirstOrDefault();

                    var testingEquipment = db.EquipmentCT_Testing.Where(item => item.CurrentTransformerId == currentTransformerId).ToList().LastOrDefault();
                    if (target != null)
                    {
