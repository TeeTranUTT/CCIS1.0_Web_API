    ...
    public class HiloController : ApiBaseController
    {
        ...
        public HttpResponseMessage ErrorHiloBillManage([DefaultValue(-1)] int departmentID, [DefaultValue("HILO")] string TypeLog)
        {
            try
            {
                ...
                {
                    var lstErrorHiloLog = db.App_Log.Where(i => i.Status == false && i.DepartmentID == departmentID && i.TypeLog == TypeLog).ToList();
                    ...
                    lstBillErr = (from a in db.Bill_ElectricityBill
                                  join b in db.Bill_ElectronicBill on a.BillId equals b.BillId
                                  join c in db.Category_FigureBook on a.FigureBookId equals c.FigureBookId
                    ...
                    lstBillErrTaxInvice = (from a in db.Bill_TaxInvoice
                                           join b in db.Bill_ElectronicBill on a.BillId equals b.BillId
                                           where listBillID.Contains(a.BillId)
                                           select new ErrorBillHilo
                                           {
                    ...
                    lstBillAdjustment = (from a in db.Bill_ElectricityBillAdjustment
                                         join b in db.Bill_ElectronicBill on a.BillId equals b.BillId
                                         where listBillID.Contains(a.BillId)
                                         select new ErrorBillHilo
                                         {
        ...
        public HttpResponseMessage PushHiloBill(PushHiloBillInput input)
        {
            try
            {
                using (var db = new CCISContext())
                {
                    ...
                    var vErrorHiloLog = db.App_Log.Where(i => i.DepartmentID == input.DepartmentId && input.BillId.Contains(i.BillID.ToString()) && i.TypeLog == AppLogTypes.HILO && i.Status == false).Select(x => x.BillID).ToList();

                    var billElectronic = (from bill in db.Bill_ElectronicBill
                                          join appLog in db.App_Log on new { A = bill.BillId, B = bill.BillType } equals new { A = appLog.BillID, B = appLog.BillType }
                    ...
                    var loginInfo = db.Administrator_Parameter.FirstOrDefault(i => i.DepartmentId == input.DepartmentId && i.ParameterName == Administrator_Parameter_Common.HILO_AUTHENTICATION);
                    if (loginInfo == null)
                    {
                    ...
                    {
                        var taxtcode = db.Category_Serial.Where(i => i.DepartmentId == input.DepartmentId).Select(i => i.TaxCode).FirstOrDefault();
        ...
        public HttpResponseMessage DeleteLog(decimal BillId)
        {
            try
            {
                using (var db = new CCISContext())
                {
                    var billLog = db.App_Log.Where(i => i.BillID == BillId && i.Status == false && i.TypeLog == AppLogTypes.HILO).FirstOrDefault();
                    if (billLog != null)
                    {
                        db.App_Log.Remove(billLog);
                        db.SaveChanges();
                    }
                    else
                    {
